...
namespace LKaifer_BugTracker.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> DemoLogin(string demoRole, string returnUrl)
        {
            ...
            {
                case "AdminDemo":
                    model.Email = "lmkaifer@hotmail.com";
                    break;
                case "ManagerDemo":
                    model.Email = "manager@mailinator.com";
                    break;
                case "DeveloperDemo":
                    model.Email = "developer@mailinator.com";
                    break;
                case "SubmitterDemo":
                    model.Email = "submitter@mailinator.com";
                    break;
            }
            if (!ModelState.IsValid)
            {
            ...
            // This doesn't count login failures towards account lockout
            // To enable password failures to trigger account lockout, change to shouldLockout: true
            var result = await SignInManager.PasswordSignInAsync(model.Email, "Coder@1234", model.RememberMe, shouldLockout: false);
            switch (result)
            {
                case SignInStatus.Success:
                    return RedirectToLocal(returnUrl);
